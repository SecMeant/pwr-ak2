...
bignum_multiply_fixed_outter_loop: 
  # i = 0
  movq $0, %r12
  ...
  movq %r13, %r14
  cmpq $0,%rsi
  jbe bignum_multiply_fixed_finish

  bignum_multiply_fixed_inner_loop:
    ...
    jnc bignum_multiply_fixed_no_carry_3
    incq %rdx # carry++
  bignum_multiply_fixed_no_carry_3:
    # add carry
    addq %rdi, (%r15, %r14,CHUNK_SIZE ) 
    movq %rdx, %rdi

    jnc bignum_multiply_fixed_no_carry_4
    incq %rdi # carry++
  bignum_multiply_fixed_no_carry_4:   
    incq %r12
    incq %r14
    cmpq %r12, %rsi
    jg bignum_multiply_fixed_inner_loop
  # prepand carry
  addq %rdi, (%r15, %r12,CHUNK_SIZE )
  # reset carry
  movq $0, %rdi  
  ...
  incq %r13
  decq %rsi
  jmp bignum_multiply_fixed_outter_loop

bignum_multiply_fixed_finish:
