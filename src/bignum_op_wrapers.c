...
bignum bignum_multiply(bignum first, bignum second){
	int64_t first_chunk_size = bignum_effective_width_chunks(first);
	int64_t second_chunk_size = bignum_effective_width_chunks(second);

	bignum res = bignum_make(first_chunk_size + second_chunk_size );

	first.bignum_size =  first_chunk_size;
	second.bignum_size =  second_chunk_size;
